...
class BooksByNrOfBooksByAuthors(APIView):
    def get(self, request, format=None):
        nrOfPublications = Publisher.objects.annotate(num_published=Count('books')).filter(num_published__gt=5).filter(established__year__gt=1928).order_by('num_published')
